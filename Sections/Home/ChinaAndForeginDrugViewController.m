//
//  ChinaAndForeginDrugViewController.m
//  jiankemall
//
...
#import "TypesLabelCell.h"
#import "ProductDetailViewController.h"
#import "MJRefresh.h"
@interface ChinaAndForeginDrugViewController ()
{
    UITableView *chinaAndForeginDrugTable;
...
@implementation ChinaAndForeginDrugViewController
...
- (void)viewDidLoad {
    ...
    chinaAndForeginDrugTable = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height) style:UITableViewStyleGrouped];
    chinaAndForeginDrugTable.delegate = self;
    chinaAndForeginDrugTable.dataSource = self;
    chinaAndForeginDrugTable.separatorStyle = UITableViewCellSeparatorStyleNone;
    chinaAndForeginDrugTable.backgroundColor = [UIColor jk_colorWithHexString:@"#f5f5f5"];
    [self.view addSubview:chinaAndForeginDrugTable];
    _dataArray = [[NSMutableArray alloc] initWithCapacity:0];
    [_dataArray addObject:[NSNumber numberWithBool:NO]];
    
    
    //注册xib
    [chinaAndForeginDrugTable registerNib:[UINib nibWithNibName:@"ChinaAndForeginDrugCell" bundle:nil] forCellReuseIdentifier:@"chinaAndForegineCell"];
    [chinaAndForeginDrugTable registerNib:[UINib nibWithNibName:@"TypesLabelCell" bundle:nil] forCellReuseIdentifier:@"labelCellOne"];
    
    // 集成中西医药刷新控件
    [self setupchainAndForieginRefresh];
    
    titleArr = @[@"解热镇痛",@"胃肠疾病",@"皮肤用药",@"风湿跌打"];
    widthValue = 10;
    flag = 0;
    value2 = 0;
    value = 0;
...
- (void)setupchainAndForieginRefresh
{
    // 上拉加载更多(进入刷新状态就会调用self的footerRereshing)
    [chinaAndForeginDrugTable addFooterWithTarget:self action:@selector(twoSexFooterRereshing)];
    
    chinaAndForeginDrugTable.footerPullToRefreshText = @"上拉加载更多";
    chinaAndForeginDrugTable.footerReleaseToRefreshText = @"松开马上加载更多";
    chinaAndForeginDrugTable.footerRefreshingText = @"正在帮你加载中.....";
}

#pragma mark 中西医请求
-(void)requestChinaAndForiegnData:(NSString *)classId{
    
    [[Loading shareLoading]beginLoading];
    NSDictionary *parameters = @{@"id":@"2",@"currentpage":[NSString stringWithFormat:@"%d",currentpage],@"pagerows":[NSString stringWithFormat:@"%d",pageRows],@"classId":classId};
    //url地址
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    currentpage = 1;
    pageRows = 10;
    [self requestChinaAndForiegnData:tempId];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    switch (indexPath.section) {
        case 0:
        {
            TypesLabelCell *cell_typeLabel = (TypesLabelCell *)[tableView dequeueReusableCellWithIdentifier:@"labelCellOne"];
            ...
            [cell_typeLabel.label3 addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(changeData:)]];
            [cell_typeLabel.label4 addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(changeData:)]];
            [cell_typeLabel.label5 addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(changeData:)]];
            [cell_typeLabel.label6 addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(changeData:)]];
            [cell_typeLabel.label7 addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(changeData:)]];
            [cell_typeLabel.label8 addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(changeData:)]];
            [cell_typeLabel.label9 addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(changeData:)]];
            [cell_typeLabel.label10 addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(changeData:)]];
            [cell_typeLabel.label11 addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(changeData:)]];
            [cell_typeLabel.label12 addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(changeData:)]];
            [cell_typeLabel.label13 addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(changeData:)]];
            [cell_typeLabel.label14 addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(changeData:)]];
...
-(void)showType:(UIButton *)btn{
    ...
    [chinaAndForeginDrugTable reloadSections:[NSIndexSet indexSetWithIndex:0] withRowAnimation:UITableViewRowAnimationAutomatic];
}
...
-(void)changeDataTwo:(UITapGestureRecognizer *)gesture{
    ...
    currentpage = 1;
    [chinaAndForeginDrugTable reloadData];
    [productInfoArr removeAllObjects];
    tempId = [NSString stringWithFormat:@"010%d",(int)gesture.view.tag-199];
    NSLog(@"tempId==111111111111111=%@",tempId);
    [self requestChinaAndForiegnData:tempId];
}
...
-(void)changeData:(UITapGestureRecognizer *)gesture{
    value = (gesture.view.tag-101)%4;
    value2 = (int)(gesture.view.tag-101)/4;
    NSLog(@"value ==== %d,value2===%d",value,value2);
    flag = 1;
    currentpage = 1;
    [chinaAndForeginDrugTable reloadData];
    [productInfoArr removeAllObjects];
    if (gesture.view.tag < 106) {
        tempId = [NSString stringWithFormat:@"010%d",(int)gesture.view.tag-96];
    }else{
        tempId = [NSString stringWithFormat:@"01%d",(int)gesture.view.tag-96];
    }
    NSLog(@"tempId-----2------%@",tempId);
    [self requestChinaAndForiegnData:tempId];
}
...
-(void)twoSexFooterRereshing{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [chinaAndForeginDrugTable footerEndRefreshing];
    });
}
